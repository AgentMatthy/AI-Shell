...
Consider multi-step tasks carefully. A single successful command doesn't mean the entire request is complete.
    """
            ...
            try:
                ...
                if command["action"] == "execute_command":
                    ...
                    if confirm == "Y":
                        ...
                        # Track conversation and check task status
                        conversation_history.append(f"Command: {command['command']}")
                        ...
                        if "[CONTINUE]" in task_status:
                            ...
                            if retry_count < max_retries:
                                ...
                                payload.append({"role": "user", "content": f"Task failed after {max_retries} attempts. Please provide a summary of what was attempted and suggest alternatives using direct_reply format."})
                                rejudge = True
                                retry_count = 0
                        else:  # [COMPLETE]
                            # Task is complete - ask for summary
                            payload.append({"role": "user", "content": f"Task completed. Please provide a brief summary of what was accomplished for: {original_request}. Use direct_reply format."})
                            rejudge = True
                            retry_count = 0
                    else:
